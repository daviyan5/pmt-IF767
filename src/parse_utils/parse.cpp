...
int algs_number = 2;
char alg_names[][30] = {"boyermoore","wumanber","bruteforce"};
...
void print_helper(){
    ...
    print_option("-s, --statistics:","Mostra estatísticas de tempo.\n");
...
void read_pattern_file(Args &pmt){
    ...
    while(getline(file,temp)){
        pmt.patterns.push_back(temp.c_str());
    }
...
Args parse_commands(int argc,char *argv[]){
    ...
    init_args(pmt_args);
    int opt;
    while((opt = getopt_long(argc, argv, "he:a:cspim:no:",long_options,NULL)) != -1){
